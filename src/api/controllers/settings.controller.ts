...
export class SettingsController {
  ...
  async getCompany(req: Request): Promise<any> {
    ...
    return await this.settingsService.getCompany(user.codigoEmpresa)
  }

  @asyncHandler()
  async updateCompany(req: Request): Promise<any> {
    const user = (req as any).user as TokenPayload
    return await this.settingsService.updateCompany(user.codigoEmpresa, String(req.body?.name ?? ''))
  }
