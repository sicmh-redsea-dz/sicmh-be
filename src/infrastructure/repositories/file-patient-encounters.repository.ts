...
const defaultStore = (): PatientEncountersStore => {
...
export class FilePatientEncountersRepository implements PatientEncountersRepository {
  async load(): Promise<PatientEncountersStore> {
    ...
    try {
      ...
      return JSON.parse(raw) as PatientEncountersStore
    } catch (err: any) {
      if (err.code === 'ENOENT') {
        const store = defaultStore()
        await fs.writeFile(FILE_PATH, JSON.stringify(store, null, 2), 'utf-8')
